...
function BandDiagram({
  ...
  const ring = (band: string, i: number, field: string) => (
    <DiagramPart key={`${band}-${i}`} field={field}>
      ...
      <path
        ...
        d={`M${ringPositions[i] + 12} 150V173`}
      />
      <text className="band-label-title" x={ringPositions[i] + 12} y="191">
        {role(i)}
      </text>
      <text className="band-label-value" x={ringPositions[i] + 12} y="212">
        {detail(band, i)}
      </text>
    </DiagramPart>
  );
  return (
    ...
    >
      ...
      <defs>
        <linearGradient id="lead-metal" x1="0" x2="0" y1="0" y2="1">
